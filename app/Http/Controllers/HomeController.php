...
class HomeController extends Controller
{
    ...
    public function index()
    {
        $defaults = [
            ...
            'topCustomersList' => collect(),
        ];
        try {
            return $this->indexDashboard();
        } catch (\Throwable $e) {
            \Illuminate\Support\Facades\Log::error('Home dashboard error: '.$e->getMessage(), [
                'trace' => $e->getTraceAsString(),
                'file' => $e->getFile(),
                'line' => $e->getLine(),
            ]);
            $message = config('app.debug') ? $e->getMessage() : 'Unable to load dashboard. Please try again or contact support.';
            try {
                return view('home', $defaults);
            } catch (\Throwable $viewEx) {
                \Illuminate\Support\Facades\Log::error('Home fallback view error: '.$viewEx->getMessage());
                abort(500, $message);
            }
        }
    }
    ...
    /**
     * Build dashboard data and return view.
     */
        ...
        $topCustomersList = $topCustomersData->map(function ($row) use ($topCustomers) {
            ...
            if ($c) {
                if (is_array($names) && count($names) > 0) {
                    $name = (string) ($names[0] ?? $c->company ?? 'N/A');
                } else {
                    $name = (string) ($c->company ?? 'N/A');
                }
            }
